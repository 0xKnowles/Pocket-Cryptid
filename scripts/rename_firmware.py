...
Release-candidate outputs when POCKET_CRYPTID_RC_ARTIFACTS=1:
  .pio/build/tiny/firmware-tiny-<branch>-<hash>-RC.bin
  .pio/build/xlarge/firmware-xlarge-<branch>-<hash>-RC.bin

Release outputs when POCKET_CRYPTID_RELEASE_VERSION is set:
...
def _get_rc_artifact_name(project_dir, env):
    ...
    branch = (
        ...
        or os.environ.get('POCKET_CRYPTID_RC_BRANCH')
        or _get_git_branch(project_dir)
    )
    short_hash = (
        _get_project_option(env, 'custom_rc_hash')
        or os.environ.get('POCKET_CRYPTID_RC_HASH')
        or _get_git_value(
        project_dir,
        'rev-parse',
        '--short',
...
def _is_rc_artifact_build(env):
    flag = _get_project_option(env, 'custom_rc_artifacts') or os.environ.get('POCKET_CRYPTID_RC_ARTIFACTS')
    return str(flag).strip().lower() in {'1', 'true', 'yes', 'on'}


def _get_release_version(env):
    version = _get_project_option(env, 'custom_release_version') or os.environ.get('POCKET_CRYPTID_RELEASE_VERSION')
    if not version:
        return None
