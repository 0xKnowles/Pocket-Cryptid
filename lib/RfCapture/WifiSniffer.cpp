...
void WifiSniffer::promiscuousRxCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  if (type == WIFI_PKT_MGMT && fType == FC_TYPE_MGMT) {
    ...
    if (subtype == FC_SUBTYPE_BEACON || subtype == FC_SUBTYPE_PROBE_RESP) {
      ...
      // transmission on the way out, so there's no self-detection concern to filter here — moot
      // anyway since DeauthEngine's own bursts are confirmed rejected by the driver before they
      // ever transmit).
